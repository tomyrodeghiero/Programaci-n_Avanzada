...
natToBinaryRev 0 = []
natToBinaryRev n | n `mod` 2 == 0 = 0 : natToBinaryRev (n `div` 2)
                 | otherwise = 1 : natToBinaryRev (n `div` 2)
...
perfectSquare n = if (n == head(reverse (divsSqrt n)) * head (reverse (divsSqrt n))) then True else False
